...
def dumpdexFile(classesFile, output):
    dir = "./dumpFile/"
    ...
    if os.path.exists(file):
        os.remove(file)
    str = "../sherlloo/build-tools/26.0.1/dexdump -d " + classesFile + " > " + file
...
def code2Hash(dump):
   codes = departCode(dump)
   # set default k to 3 and base(b) to 8
   k = 3
   b = 8
   hashes = []
...
def calculateDistance(hash0, hash1):
    #percentage = 1/len(hash0)
    dict0 = {}
    dict1 = {}
    for h in hash0:
        if h in dict0.keys():
    ...
    score = 0.0
    total = len(hash0) + len(hash1)
    for d in dict0.keys():
        if d in dict1.keys():
            score = score + (dict0[d] + dict1[d])/total

    return (dict0, dict1, score)


if __name__ == "__main__":
    classes0 = "./ClassesFiles/baseline/classes.dex"
    classes1 = "./ClassesFiles/obfuscated/classes.dex"
    dump0 = "dump0.txt"
    dump1 = "dump1.txt"
    dumpdexFile(classes0, dump0)
    dumpdexFile(classes1, dump1)
    hash0 = code2Hash(dump0)
    hash1 = code2Hash(dump1)
    result0, result1, score = calculateDistance(hash0, hash1)
    print(result0)
    print(result1)
    print(score)